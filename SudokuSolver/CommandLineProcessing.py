...
class CommandLineProcessing:
    ...
    def processing(self):
        ...
        cmd.add_argument("--s",default=False,help="Solve on startup?: y/n ignored if --f is None")
        cmd.add_argument("--t",default=0,help="Float amount for time delay ignored if --f or --s is None")
        cmd.add_argument("--n",default=None,help="Name of the file to save puzzle state to ignored ignored if --f or --s is None")
        cmd.add_argument("--e",default=False,help="Exit on solve? y/n ignored if --n or --s is None")
        ...
        if parser.filename == None: #if no filename skip otherwise if --s is set then set solve_on_startup accordingly 
            ...
            pass
        if args.t != 0 and args.t > 0: #if not the default and a non negative 
